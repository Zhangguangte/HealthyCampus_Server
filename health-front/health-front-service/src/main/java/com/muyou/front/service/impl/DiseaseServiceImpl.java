...
//					DISEASE_CLASS_LIST + ":" + requestForm.getQuest_id() + ":" + requestForm.getContent(),
...
//				jedisClient.rpush(DISEASE_CLASS_LIST + ":" + requestForm.getQuest_id() + ":" + requestForm.getContent(),
...
//	}
	...
	public DiseaseDetailVo getDiseaseDetail(RequestForm requestForm) {
		...
		try {
			...
			if ("NAME".equals(requestForm.getQuest_id())) {
				jedisClient.expire(DISEASE_DETAIL + ":" + requestForm.getQuest_id(), 60 * 60 * 24 * 10);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
