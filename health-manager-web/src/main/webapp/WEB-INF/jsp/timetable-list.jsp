...
<div class="pos-a" style="width:200px;left:0;top:0; bottom:0; height:100%; border-right:1px solid #e5e5e5; background-color:#f5f5f5; overflow:auto;">
    <ul style="margin-top: 15px;margin-left: 20px">
    	<i class="Hui-iconfont Hui-iconfont-fenlei"></i> 课表分类</ul>
...
<div style="margin-left:200px;">
    ...
    <form id="form-search" class="page-container">
        ...
        <div class="cl pd-5 bg-1 bk-gray mt-20"> 
        	<span class="l">
        		<a class="btn btn-primary radius" onclick="timetable_add('添加课表','timetable-add')" href="javascript:;">
        			<i class="Hui-iconfont">&#xe600;</i> 添加课表
        		</a>
        	</span>
        </div>
...
<script type="text/javascript">
        ...
        callback: {
            ...
            beforeClick: function(treeId, treeNode) {
                if (treeNode.isParent) {
                    ...
                    $("#cYear option[value='2020']").prop("selected",true);
                    $("#semester option[value='1']").prop("selected",true);
    ...
    /*课表-添加*/
    function timetable_add(title,url){
    	if($("#category").html() == "所有课表"){
    		layer.msg('请您选择一名教师!',{icon:5,time:3000});
    		return ;
    	}
    ...
    /*课表-编辑*/
    function timetable_edit(title,url){
    	var index = layer.open({
            type: 2,
            title: title,
            content: url
    ...
    var COURSE = {
    	loadCourse : function(){
    	    ...
    	    Timetable = new Timetables({
    	       ...
    	       gridOnClick: function (e) {
    	       	...
    	       	var index = layer.confirm(e.name + '  ' + e.week + ', 第' + e.index + '节课, 课长' + e.length + '节', {
    	               btn: ["修改",'删除',"取消"], //按钮
    	               shade: false //不显示遮罩
    	           }, function(){
    	        	   setId(ids[COURSE.weeks(e.week)][e.index-1]);
    	        	   timetable_edit("e.name","timetable-edit");
    	        	   layer.close(index);
    	           }, function(){
    	           	layer.confirm('确认要删除\''+ e.week + ', 第' + e.index + '节课, 课长' + e.length + '节\'的课吗？',{icon:3},function(index){
    	           		COURSE.delItem(ids[COURSE.weeks(e.week)][e.index-1]);
    	           		layer.close(index);
    	           	});
    	           }, function(){
    	           });
    	...
    	getCourseList : function(){
    		loading(true);
    		$.ajax({
                url:"/item/timetable/list/"+cid+"/"+$("#cYear").val()+"/"+$("#semester").val(),
                type: 'GET',
                dataType: "json",
