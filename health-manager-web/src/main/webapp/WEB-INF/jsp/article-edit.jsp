...
<div class="page-container">
    <form name="article-add" action="" method="post" class="form form-horizontal" id="article-add">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <textarea value="" class="textarea"  placeholder="简介字数在100之内" id="introduction" name="introduction" style="min-height:80px;resize:none;"></textarea>
            </div>
        </div>
        <div class="row cl">
            <input type="text" hidden class="input-text" id="picUrl" name="picUrl" >
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <textarea id="content" name="content" style="width:100%;visibility:hidden;"></textarea>
            </div>
        </div>
        <div class="row cl">
...
<script type="text/javascript">
	...
	$("#article-add").validate({
	    ...
	    submitHandler:function(form){
	        editor.sync();
	        if("" == $("#content").val()){
                layer.alert('请填写内容! ', {title: '错误信息',icon: 0});
                return;
            }
	        var index1 = layer.load(3);
	        $(form).ajaxSubmit({
	            url: "/article/update/"+parent.getId(),
	            type: "POST",
	            dataType: "json",
    ...
    KindEditor.ready(function(K) {
        editor = K.create('textarea[name="content"]', {
            ...
            afterCreate : function() {
                ...
                K.ctrl(document, 13, function() {
                    self.sync();
                    document.forms['article-add'].submit();
                });
        ...
        $.ajax({
            type: 'GET',
            url: '/article/'+parent.getId(),
            dataType: 'json',
            success: function(data){
                layer.close(index);
                $("#title").val(data.result.title);
                $("#introduction").val(data.result.introduction);
                if(null ==  data.result.picUrl || "" == data.result.picUrl)
                	data.result.picUrl = "http://192.168.2.134/group1/M00/00/00/wKgChl5vp1mACUPXAAA1_Y-pCUM381.png";
                $("#picUrl").val(data.result.picUrl);
                $("#imageShow").attr("src",data.result.picUrl);
                $("#imageUrl").attr("href",data.result.picUrl);
                htmlData=data.result.content;
                KindEditor.html('#content', htmlData);
            },
            error:function(XMLHttpRequest) {
                layer.close(index);
                layer.alert('数据处理失败! 错误码:'+XMLHttpRequest.status+' 错误信息:'+JSON.parse(XMLHttpRequest.responseText).message,{title: '错误信息',icon: 2});
