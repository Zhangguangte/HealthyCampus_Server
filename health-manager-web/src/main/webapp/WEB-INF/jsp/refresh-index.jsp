...
<div style="margin-left: 1vw;margin-right: 1vw" class="cl pd-5 bg-1 bk-gray mt-20"> 
	<span class="l"> 
		<a class="btn btn-primary radius" onclick="refresh_disease()" href="javascript:;"><i class="Hui-iconfont">&#xe645;</i> 同步疾病索引</a>
	</span> 
	<span class="l" style="margin-left:10px;margin-right:10px;"> 
		<a class="btn btn-primary radius" onclick="refresh_medicine()" href="javascript:;"><i class="Hui-iconfont">&#xe645;</i> 同步药品索引</a>
	</span> 
</div>
<table class="table">
    <tr>
...
<script type="text/javascript">
    ...
    function getInfo() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#cluster_name").val(data.result.clusterName);
                $("#number_of_nodes").val(data.result.nodesNum);
                $("#count").val(data.result.docStr);
            },
    ...
    /*同步疾病索引*/
    function refresh_disease(){
        layer.confirm('确认要同步数据库中疾病数据至SolrCloud索引库吗？',{icon:3},function(index){
            var index = layer.load(3);
            $.ajax({
                type: 'GET',
                url: '/search/disease/importDisease',
                dataType: 'json',
                success: function(data) {
                    layer.close(index);
                    if(data.success!=true){
    ...
    /*同步药品索引*/
    function refresh_medicine(){
        layer.confirm('确认要同步数据库中药品数据至SolrCloud索引库吗？',{icon:3},function(index){
            var index = layer.load(3);
            $.ajax({
                type: 'GET',
                url: '/search/medicine/importMedicine',
                dataType: 'json',
                success: function(data) {
                    layer.close(index);
                    if(data.success!=true){
